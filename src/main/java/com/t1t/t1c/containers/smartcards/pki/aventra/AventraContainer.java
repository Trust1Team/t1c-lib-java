...
public class AventraContainer extends SmartCardContainer<AventraContainer, GclAventraRestClient, AventraAllData, AventraAllCertificates> {
    ...
    public AventraAllData getAllData(List<String> filterParams, Boolean parseCertificates) throws GenericContainerException {
        return new AventraAllData(RestExecutor.returnData(httpClient.getAllData(getContainerVersionId(), reader.getId(), createFilterParams(filterParams)), config.isConsentRequired()), parseCertificates);
    }
    ...
    public AventraAllCertificates getAllCertificates(List<String> filterParams, Boolean parseCertificates) throws GenericContainerException {
        return new AventraAllCertificates(RestExecutor.returnData(httpClient.getAllCertificates(getContainerVersionId(), reader.getId(), createFilterParams(filterParams)), config.isConsentRequired()), parseCertificates);
    }
    ...
    public Boolean resetPin(String puk, String pin, String keyRef) throws RestException {
        try {
            ...
            return RestExecutor.isCallSuccessful(RestExecutor.executeCall(httpClient.resetPin(getContainerVersionId(), reader.getId(), new GclAventraPinResetRequest().withNewPin(pin).withPuk(puk).withPrivateKeyReference(keyRef)), config.isConsentRequired()));
        } catch (RestException ex) {
            throw PinUtil.checkPinExceptionMessage(ex);
        }
    }
